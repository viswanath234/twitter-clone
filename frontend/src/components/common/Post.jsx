  ...
  const { mutate: likePost, isPending: isLiking } = useMutation({
    mutationFn: async () => {
      try {
        ...
        const data = await res.json();
        if (!res.ok) {
          throw new Error(data.errror || "Something went wrong");
        }
        return data;
      } catch (error) {
        throw error;
    ...
    onSuccess: (updatedLikes) => {
      ...
      // queryClient.invalidateQueries({ queryKey: ["posts"] }); - not correct wayy
      // instead  update the cache directly on the post
      queryClient.setQueryData(["posts"], (oldData) => {
        //olddata refers to updater
